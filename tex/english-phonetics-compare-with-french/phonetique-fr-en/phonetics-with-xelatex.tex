...
\exEN{If you want more examples with the word ``#1'' click on the flags below:}%
  \begin{center}%
...
\newcommand{\HTEBL}{%
    ~\href{https://www.amazon.fr/gp/product/B00M0GDXN8/ref=as_li_tl?ie=UTF8&camp=1642&creative=6746&creativeASIN=B00M0GDXN8&linkCode=as2&tag=wwwbecomefree-21&linkId=e782cf430858413ec9135fcef1644b20}{How The Ell Brain Learns}\xspace%
}
...
\newcommand{\indicsound}{%
  ...
  %
  \begin{itemize}%
  \item \exEN{Click on the flags below in order to check how to produce the sound properly.}%
...
%%%%%%%%%%%%%%%
\newcommand{\speech}[2]{%
  Il y a #1 #2 dans la langue anglaise et pour chaque son, je vous~%
  proposerai au moins 4~%
  exemples. Pour chaque son je commencerai par vous donner sa définition~%
  ...
  explique la façon de produire le son. Vous aurez également deux liens~%
  externes :%
  %
  \begin{enumerate}%
  \item Le premier lien\footnote{Accessible en cliquant sur le drapeau du~%
      Royaume Uni.} vous permettra de consulter une vidéo qui~%
  ...
  \item Le second lien\footnote{Accessible en cliquant sur le drapeau~américain.}~%
    vous permettra de consulter une vidéo qui propose des~%
    exemples supplémentaires avec une prononciation typiquement américaine.%
  \end{enumerate}%
  %
  La structure de chaque section sera toujours la même pour chaque son :%
  \begin{enumerate}%
  \item Des exemples de mots afin d'illustrer le son. Pour chaque mot~%
    vous disposerez d'un lien externe vers un dictionnaire~%
    \exEN{anglais}-\exFR{français} qui vous proposera plusieurs prononciation, la~%
    phonétique, la traduction\footnote{En fait
      plusieurs,~avec~parfois~plusieurs registres de langue (y compris l'argot).}, et enfin~%
  ...
  \item Des exemples de phrases utilisant le mot. Dans chaque phrase~%
    ...
    des locuteurs natifs\footnote{Souvent américains.}.%
  \item Des exemples complémentaires\footnote{Avec cette fois en bonus des~%
      liens vers des prononciation typiquement australiennes en plus des~%
...
\begin{document}
\title{Introduction à la phonétique anglaise comparée à la française; rédigée avec \href{https://en.wikipedia.org/wiki/XeTeX}{\XeLaTeX}}
\hypersetup{
